...
//	}
	...
	private void reset() {
		...
		for (int i = 0; i < mShearGeometries.size(); i++) {
			mSplitters.add(SplitterFactory.createSplitter(mShearGeometries
					.get(i)));
		}
	...
	/*public double[] getShearArea(){
		...
		if (mShearGeometries != null && mShearGeometries.size() > 1){
			int mGeos = mShearGeometries.size();
			IGeometry geo = mShearGeometries.get(mGeos - 1);
			IGeometry geo1 = mShearGeometries.get(mGeos - 2);
			areas[0] = ((Polygon)geo).Area();
			System.out.println("===========最后" + areas[0]);
			areas[1] = ((Polygon)geo1).Area();
			System.out.println("===========倒数第二" + areas[1]);
			return areas;
		}
		return areas;
	}*/
